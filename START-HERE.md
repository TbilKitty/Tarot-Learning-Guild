...
The site is complete as a **free learning guide and unlimited five-card written practice tool**. When you connect the services below, visitors get **three free personalized AI readings in their browser**, then pay **$2 for each additional AI reading**. The free and paid AI readings each include one question, five main cards, and one shared set of three clarifiers for the full spread (8 distinct cards total). Cards and AI answers are saved for 30 days. There is no visitor account system; see the limits in section 5.

## 1. Look at it tonight

1. Unzip `Tarot-Learning-Guild.zip` on a computer.
2. Keep `Fifth-Card-Preview.html` and `Tarot-Learning-Guild-Card-Meanings.html` in the same folder. Double-click `Fifth-Card-Preview.html` to open the working offline preview. Follow the **Card meanings** link to explore the separate 78-card reference page. Try **Love**, **Job / Career**, **Money**, and **Other**. Draw a practice spread and click **Reveal 3 clarifiers for the full spread** beneath the five cards.
...
You already have GitHub Pages sites, so you can use the same approach. Open the ZIP, then open the `public` folder. Upload the **contents of `public`**, including `index.html`, `card-meanings.html`, `style.css`, `app.mjs`, `meanings.mjs`, `deck.mjs`, `card-back.webp`, and `.nojekyll`, to the root of a **new public GitHub repository**. If your computer hides `.nojekyll`, the site still works; GitHub Pages may process the files through Jekyll, so add a blank `.nojekyll` file at the repository root when you can. The `_headers` file is for Cloudflare; GitHub Pages ignores it.

In the repository, go to **Settings → Pages → Build and deployment → Deploy from a branch**. Choose `main` and `/ (root)`, then save. Open the GitHub Pages URL once it reports the site is live. Check the free practice draw, clarifier buttons, and the **Card meanings** navigation link on your phone. You can share the free guide at this point. GitHub Pages cannot securely run the included purchase and AI code by itself.
...
- The 78 original short meanings live in `deck.mjs`. The separately linked reference page is `card-meanings.html`; its 78 entries and individual reflection prompts are built by `build.mjs`. The search interaction is `meanings.mjs`.
...
The home page has an original learning lesson and a worked example. A separate **Card Meanings** page groups all 78 cards by suit, with original upright and reversed descriptions and a question for each card. The build writes that content directly into the HTML, and ordinary links connect both pages so search engines can find them. Add original depth over time based on what your readers actually want to learn; more pages or ads cannot guarantee search ranking. To compare names, go to [Google Trends Explore](https://trends.google.com/trends/explore), set the same country, date range, and search type, and compare phrases such as **learn tarot**, **how to read tarot cards**, **tarot card meanings**, and **tarot learning guide**. Trends displays *relative interest*, not the total number of searches; do not treat a single score as a traffic forecast. [Google's explanation](https://support.google.com/trends/answer/4365533?hl=en) covers normalization.
...
To see whether the reference page appears in Google searches, verify the final domain in [Google Search Console](https://search.google.com/search-console/). Use its **Pages** and **Queries** reports to compare the homepage and `card-meanings.html` once Google has had time to discover them. Google Analytics measures visitors and their behavior **after** you install its tag; it does not submit a page to search results. The package has no tracking tag yet, so create a separate Analytics property for this site before adding one. When you know your final hostname, you can also submit both published URLs in Search Console. Avoid submitting a temporary preview URL as the permanent site.

For buying search ads, start with a measured test only after both real purchases and the free-to-paid conversion can be counted. Google Ads Keyword Planner offers local estimates for clicks and cost, but an ad budget does not buy the top organic result. With a $2 product and three free AI readings before payment, even a $0.10 click would require **at least** a 5% eventual paid conversion rate just to cover the ad click before payment fees and AI costs. If the measured cost per click is above your break-even amount, pause paid ads and focus on improving the free guide and organic discovery.
